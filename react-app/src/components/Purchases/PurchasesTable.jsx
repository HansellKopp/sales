...
import Purchase from 'components/Purchase/PurchaseForm/PurchaseForm'
import EnhancedTableToolbar from './EnhancedTableToolbar'

import { getPurchasesDates, getPurchase } from 'store/slices/documentSlice'
import { getComparator, stableSort, formatNumber, formatDate, today } from 'utils/utils'
import { purchaseTableHead } from 'store/mockups/settings.json'
...
export default function EnhancedTable() {
  ...
  const rows = useSelector(state => state.document.purchases)
  const { readyToPrint } = useSelector(state => state.document.purchase)
  const [range, setRange] = React.useState({from: today(), to: today()});
  const dispatch = useDispatch()

  useEffect(() => {
    dispatch(getPurchasesDates({ from: range.from, to: range.to }))
  ...
  useEffect(() => {
    if(readyToPrint) {
      doPrint()
      dispatch({ type: 'document/clearPurchase' })
    }
    // eslint-disable-next-line 
  }, [readyToPrint])
  ...
  const printItem = () => {
    ...
    const purchase = rows.find(s=> s.id===item)
    if(purchase) {
      dispatch(getPurchase(purchase.id))
      setSelected([])
    }
  }
  ...
  const totalPurchases = rows.reduce((acc, row)=> (acc+row.total), 0)
  const totalPurchasesBs = rows.reduce((acc, row)=> (acc+(row.total*row.exchange)), 0)
  ...
  return (
    <div className={classes.root}>
          ...
          >
            ...
            <TableBody>
              ...
              <TableRow>
                ...
                <TableCell align="right">{formatNumber(totalPurchases)}</TableCell>
                <TableCell align="right">{formatNumber(totalPurchasesBs)}</TableCell>
              </TableRow>
              {emptyRows > 0 && (
      ...
      />
      <div style={{ display: "none" }}><Purchase  ref={componentRef} /></div>
    </div>
  );
}
