      ...
      <Paper className={classes.paper}>
        ...
        <TableContainer className={classes.container} ref={componentRef}>
          {invoices && invoices.length > 0 ? (
            ...
            >
              <EnhancedTableHead
                headCells={summaryTableHead.invoices}
                title={`Facturas ${formatedDate}`}
              />
              <TableBody>
                {(invoices || []).map((row) => (
                  <TableRow key={row.number}>
                    ...
                    <TableCell size="small" align="right">
                      {formatNumber(row.exchange)}
                    </TableCell>
                    <TableCell size="small" align="right">
                      {formatNumber(row.total)}
                    </TableCell>
                    {/* <TableCell size="small" align="right">
                      {formatNumber(row.total_bs)}
                    </TableCell> */}
                  </TableRow>
                ))}
                <TableRow>
                  ...
                  <TableCell size="small" align="right">
                    {formatNumber(totalize(invoices, "total"))}
                  </TableCell>
                  {/* <TableCell size="small" align="right">
                    {formatNumber(totalize(invoices, "total_bs"))}
                  </TableCell> */}
                </TableRow>
              </TableBody>
              <EnhancedTableHead
                headCells={summaryTableHead.payments}
                title={"Totales por forma de pago"}
              />
              <TableBody>
                {(payments || []).map((row) => (
                ...
                <TableRow>
                  ...
                  <TableCell size="small" align="right">
                    {formatNumber(totalize(payments, "total"))}
                  </TableCell>
                  <TableCell size="small" align="right">
                    {formatNumber(totalize(payments, "total_bs"))}
                  </TableCell>
                </TableRow>
              </TableBody>
              <EnhancedTableHead
                headCells={summaryTableHead.products}
                title={"Ventas por producto"}
              />
              <TableBody>
                {(products || []).map((row) => (
                  <TableRow key={row.description}>
                    <TableCell size="medium" colSpan={4}>
                      {row.description}
                    </TableCell>
                    <TableCell size="small" align="right">
              ...
              <TableRow>
                ...
                <TableCell size="small" align="right">
                  {formatNumber(totalize(products, "quantity"))}
                </TableCell>
              </TableRow>
            </Table>
          ) : (
